 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_pinned_renderer extends format_section_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        // Since format_pinned_renderer::section_edit_control_items() only displays the 'Highlight' control when editing mode is on
    ...
    public function print_course_summary($course) : void {
        $config = get_config('format_pinned');
        ...
        $templatecontext->progresstitle = get_string('progresstitle:course', 'format_pinned');
        ...
        if (has_capability('moodle/course:update', context_course::instance($course->id))
            ...
            $messsage = get_string('nocompletion', 'format_pinned', $url->raw_out());
        ...
        if (empty($courseformatoptions['reversedisplay'])) {
            echo $this->render_from_template('format_pinned/course_summary', $templatecontext);
        } else {
            echo $this->render_from_template('format_pinned/course_summary_reverse', $templatecontext);
        }
    }
    ...
    public function print_single_section_page($course, $sections, $mods, $modnames, $modnamesused, $displaysection) {
        ...
        $templatecontext->progresstitle = get_string('progresstitle:section', 'format_pinned');

        echo $this->render_from_template('format_pinned/course_topic', $templatecontext);
    }
    ...
    protected function get_nav_links($course, $sections, $sectionno) {
        ...
        while ($back >= 0 and empty($links['previous'])) {
            if ($canviewhidden || $sections[$back]->uservisible) {
                ...
                $previouslink .= get_string('previous', 'format_pinned') . ' ';
        ...
        while ($forward <= $numsections and empty($links['next'])) {
            if ($canviewhidden || $sections[$forward]->uservisible) {
                ...
                $nextlink = get_string('next', 'format_pinned') . ' ';
